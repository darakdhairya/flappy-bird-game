...
def make_tone(frequency, duration_ms, volume=0.3, sample_rate=44100):
    """
    Build a simple sine-wave tone as a pygame Sound.
    frequency: pitch in Hz (higher = higher pitched)
    duration_ms: how long the sound lasts
    """
    n_samples = int(sample_rate * duration_ms / 1000)
    t = np.linspace(0, duration_ms / 1000, n_samples, False)
    wave = np.sin(frequency * t * 2 * np.pi)
 
    # Fade out at the end so it doesn't click/pop
...
def make_sweep(start_freq, end_freq, duration_ms, volume=0.3, sample_rate=44100):
    """Like make_tone, but the pitch slides from start_freq to end_freq."""
    n_samples = int(sample_rate * duration_ms / 1000)
    t = np.linspace(0, duration_ms / 1000, n_samples, False)
...
# A quick upward blip for flapping, a bright ding for scoring,
# and a downward sweep for crashing.
