...
# ---------- Bird image ----------
# Put a picture named "bird.png" in the same folder as this script,
# and it'll be used instead of the drawn bird shape. Works best with
# a square-ish image with a transparent or simple background.
BIRD_IMAGE_FILE = "bird.png"
bird_image = None
 
if os.path.exists(BIRD_IMAGE_FILE):
    try:
        loaded = pygame.image.load(BIRD_IMAGE_FILE).convert_alpha()
        # Scale it to roughly match the old bird's size
        target_size = bird_radius * 3
        bird_image = pygame.transform.smoothscale(loaded, (target_size, target_size))
    except pygame.error:
        bird_image = None  # fall back to the drawn bird if the file is bad
 
# Pipes
pipe_width = 60
pipe_gap = 150
BASE_PIPE_SPEED = 3
MAX_PIPE_SPEED = 7          # cap so it never becomes unfairly fast
...
def get_pipe_speed():
    """
    Pipes get gradually faster as score increases, capped at
    MAX_PIPE_SPEED so late-game doesn't become impossible.
    +0.15 per point is a gentle enough ramp to feel fair.
    """
    return min(BASE_PIPE_SPEED + score * 0.15, MAX_PIPE_SPEED)
...
def draw_bird():
    """
    If a bird.png image was successfully loaded, draw and rotate that
    image instead of the drawn shape. Otherwise, fall back to a simple
    drawn bird: oval body, a lighter wing, a white eye with a black
    pupil, and an orange triangular beak.
    """
    ...
    # Body - a wide oval rather than a plain circle
    body_rect = pygame.Rect(0, 0, bird_radius * 2.2, bird_radius * 1.7)
    body_rect.center = (cx, cy)
    pygame.draw.ellipse(bird_surface, BIRD_COLOR, body_rect)
    pygame.draw.ellipse(bird_surface, BIRD_OUTLINE, body_rect, 2)
 
    # Wing - smaller oval, slightly darker, toward the back of the body
    wing_rect = pygame.Rect(0, 0, bird_radius * 1.1, bird_radius * 0.9)
    wing_rect.center = (cx - bird_radius * 0.3, cy + bird_radius * 0.2)
    pygame.draw.ellipse(bird_surface, BIRD_OUTLINE, wing_rect)
 
    # Beak - orange triangle pointing forward (toward +x, the direction of travel)
    beak_color = (255, 140, 0)
    beak_tip = (cx + bird_radius * 1.6, cy)
    beak_top = (cx + bird_radius * 0.9, cy - bird_radius * 0.35)
    beak_bottom = (cx + bird_radius * 0.9, cy + bird_radius * 0.35)
    pygame.draw.polygon(bird_surface, beak_color, [beak_tip, beak_top, beak_bottom])
 
    # Eye - white circle with a black pupil, near the front-top of the body
    eye_center = (int(cx + bird_radius * 0.5), int(cy - bird_radius * 0.4))
    pygame.draw.circle(bird_surface, WHITE, eye_center, max(3, int(bird_radius * 0.35)))
    pygame.draw.circle(bird_surface, (20, 20, 20), eye_center, max(2, int(bird_radius * 0.18)))
