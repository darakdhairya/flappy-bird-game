...
def get_bird_angle():
    """
    Convert current velocity into a rotation angle for the bird.
    Rising (negative velocity) tilts the bird up; falling tilts it down.
    Clamped so it doesn't spin past reasonable-looking angles.
    """
    angle = -velocity * 4  # scale factor chosen by feel, not a strict formula
...
def draw_bird():
    """
    Draw the bird as a small rotated surface instead of directly onto
    the screen, so we can apply rotation based on velocity/angle.
    """
